...
import {Spinner, SpinnerExtraProps, Alert2 as Alert} from '@/packages/amis-ui/src';
import {
  autobind,
  isObject,
  isEmpty,
...
export interface ServiceSchema extends BaseSchema, SpinnerExtraProps {
  ...
  /**
   * 页面初始化的时候，可以设置一个 API 让其取拉取，发送数据会携带当前 data Content（包含地址栏参数），获取得数据会合并到 data 中，供组件内使用。
   */
  api?: SchemaApi;
