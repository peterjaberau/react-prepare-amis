...
export type SchemaQuickEditObject =
  ...
  | ({
      ...
      /**
       * 配置刷新目标，默认就会刷新所属 crud Components，
       * 如果不需要，请配置为 "none"
       */
      reload?: string;
    ...
    > {
      ...
      renderInlineForm() {
        ...
        return render('inline-form', schema, {
          ...
          disabled,
          defaultStatic: false,
          // 不下发这下面的属性，否则当使用表格类型的 Picker 时（或其他会用到 Table 的Custom Components），会导致一些异常行为
          buildItemProps: null,
...
export function InlineFormItem(
  ...
  return render('inline-form-item', schema, {
    ...
    defaultStatic: false,
    // 不下发下面的属性，否则当使用表格类型的 Picker 时（或其他会用到 Table 的Custom Components），会导致一些异常行为
    buildItemProps: null,
    quickEditFormRef: null,
    quickEditFormItemRef: null
  });
