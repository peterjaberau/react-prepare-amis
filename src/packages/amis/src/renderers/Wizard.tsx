...
import {Steps} from '@/packages/amis-ui/src';
import {
  BaseSchema,
  FormSchema,
  SchemaApi,
...
export type WizardStepSchema = Omit<FormSchema, 'type'> &
  StepSchema & {
    ...
    /**
     * 默认表单提交自己会通过发送 api 保存数据，但是也可以设定另外一个 form 的 name 值，或者另外一个 `CRUD` 模型的 name 值。 如果 target 目标是一个 `Form` ，则目标 `Form` 会重新触发 `initApi` 和 `schemaApi`，api 可以拿到当前 form Content。如果目标是一个 `CRUD` 模型，则目标模型会重新触发搜索，参数为当前 Form Content。
     */
    target?: string;
  };
...
export default class Wizard extends React.Component<WizardProps, WizardState> {
  ...
  handleAction(
    ...
    if (
      ...
      if (!action.api) {
        return env.alert(`当 actionType 为 ajax 时，请设置 api Properties`);
      }
