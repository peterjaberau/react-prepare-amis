/**
 * @file Each renderer's node information is stored here.
 * Outline is generated by this.
 */
import {
  getParent,
  getRoot,
...
import {filterSchema} from '@/packages/amis-ui/src';
...
export const EditorNode = types
  .model('EditorNode', {
    //Record father node information
    parentId: '',
    parentRegion: '',
    ...
    // A virtual renderer editor, which does not have a corresponding renderer, but is simply a subcomponent of a renderer
    memberIndex: -1,

    //Whether to collapse
    folded: false,

    // Whether the corresponding schema has been modified
    patched: false,
  ...
  .views(self => {
    ...
    return {
      ...
      getNodeById(id: string, regionOrType?: string) {
        // If not found, recursively search from root.children
        ...
        while (pool.length) {
          ...
          //Put all the child nodes of the current node into the pool
      ...
      // Is it possible to move upwards?
      get canMoveUp() {
        if (
          !this.isRegion &&
          this.moveable &&
      ...
      // Is it possible to move downwards?
      get canMoveDown() {
        if (
          !this.isRegion &&
          this.moveable &&
      ...
      // This means whether this node has one and only one container.
      // If this is the case, the container node will not be displayed in the navigation
      get singleRegion() {
        return !!(
  ...
  .actions(self => {
    // Used to unify the format of container members, because rendering supports member configuration as strings,
    // In order to be editable, it must become an object.
    ...
    /** Get the upper node through callback function*/
    ...
    // Putting it in props will make it frozen.
    let component: any;
    ...
    return {
      ...
      patch(
        ...
        // The id is repeated, regenerate one
      ...
      /**
       * Calculate the highlighted area information.
       * @param layer
       * @param root
       */
