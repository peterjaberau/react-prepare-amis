...
export interface FormulaEditorProps extends ThemeProps, LocaleProps {
  ...
  /**
   * Top title, default is expression
   */
  header: string;
  ...
  /**
   * Current input field name: used to avoid loop binding itself and causing infinite rendering
   */
...
> {
  ...
  /**
   * Can't get rid of it, it's been used in too many places, but it needs to be discarded.
   * Don't use it anymore, the input box has been replaced with codemirror, which supports highlighting
   * @deprecated
   * @param value
   * @param variables
   * @param evalMode
  ...
  static highlightValue(
    ...
    vars.forEach(v => {
      ...
      while (~(idx = content.indexOf(v, from))) {
        ...
        // If a match is found, highlight it; if no match is found, replace it with the original value
  ...
  handleVariableSelect(item: VariableItem) {
    ...
    if (
      ...
      toast.warning(
        'Cannot use the current variable [self], avoid circular references.'
      );
      return;
    }
  ...
  handleIsCodeModeChange(showCode: boolean) {
    ...
    // Non-source code mode, then mark it
  ...
  async runCode() {
    ...
    try {
      // Because resolveVariableAndFilterForAsync will not report syntax errors
      parse(value, {
        evalMode: this.props.evalMode
      });
