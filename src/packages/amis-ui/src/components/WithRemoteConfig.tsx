/**
 * A HOC that can pull remote configuration
 *
 */
...
export interface WithRemoteConfigSettings {
  ...
  /**
   * Adapt the data returned from the interface to the configuration
   */
  ...
  /**
   * Callback after request returns
   */
  afterLoad?: (ret: any, config: any, props: any) => void;

  /**
   * Lazy loading option related, callback to start lazy loading
   */
  beforeDeferLoad?: (
    item: any,
  ...
  /**
   * Lazy loading option related, callback to end lazy loading
   */
  afterDeferLoad?: (
    item: any,
    indexes: Array<number>,
      ...
      withStore(() => Store.create())(
        ...
        > {
            ...
            if (isPureVariable(source)) {
              this.toDispose.push(
                reaction(
                  ...
                  () => this.syncConfig(),
                  // When nav is configured with source: "${amisStore.app.portalNavs}", switching pages will trigger source updates
                  // Therefore, add this configuration here and execute loadConfig when the data source is completely unequal
                  // Otherwise, the data source will be reset and the expansion state will not be saved, and the accordion mode will always be
          ...
          async loadConfig(ctx = this.props.data) {
            ...
            // If source is configured: ${nav} isEffectiveApi will also be considered as an invalid request from the api
            if (isPureVariable(source)) {
          ...
          async deferLoadConfig(item: any) {
            ...
            const ret2 = await config.afterDeferLoad?.(
              item,
              indexes, // can only assume that it is still the same index
              response,
              store.config,
              this.props
            );
