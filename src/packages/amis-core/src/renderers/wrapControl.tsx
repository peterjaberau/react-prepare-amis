      ...
      observer(
        class extends React.Component<OuterProps> {
          ...
          constructor(props: OuterProps) {
            ...
            this.model = model;
            // @issue I want to get rid of this
            formItem?.addSubFormItem(model);
            model.config({
              // In theory, all defaultProps of the renderer need to take effect. Here, for safety reasons, we only process multiple
              ...pick(
            ...
            // The issue logic should be implemented in combo itself.
            if (
              this.model.unique &&
            ...
            if (propValue !== undefined && propValue !== null) {
              // Synchronize value: prioritize the value in props
            ...
            if (
              ...
              // Component default value support expressions need to: avoid losing component default values in the context during initialization
              if (model.extraName) {
              ...
              // todo This judgment will be optimized later.
              // Currently, the default value in input-table will be flushed, so this judgment is added
              // The corresponding issue is https://github.com/baidu/amis/issues/2674
              store?.storeType !== TableStore.name
            ) {
              // If there is no initial value, set it via onChange
              if (model.extraName) {
                const values = model.splitExtraValue(model.tmpValue);
                onChange(values[0], model.name, false, true);
                onChange(values[1], model.extraName, false, true);
          ...
          componentDidMount() {
            ...
            // Execute the previous lazyEmit before submitting.
          ...
          componentDidUpdate(prevProps: OuterProps) {
            ...
            // Here we need to consider both defaultValue and value
            if (typeof props.value !== 'undefined') {
              // value in the renderer takes precedence
              if (
                !isEqual(props.value, prevProps.value) &&
                !isEqual(props.value, model.tmpValue)
              ) {
                // value directly passed in from outside does not need to execute the operator
              ...
              if (
                ...
                // Identify context changes, own value changes, and formula calculation result changes
              ...
              // When value is not a formula expression, name value takes precedence. If defaultValue changes actively, defaultValue is used
              if (
                // Then check whether the associated name attribute value has changed
                props.data !== prevProps.data &&
                (!model.emitedValue ||
          ...
          setInitialValue(value: any) {
            ...
            model.changeTmpValue(
              initialValue ?? value, // Operate on component default values
          ...
          controlRef(control: any) {
            ...
            // Because control may be wrapped by n layers of hoc.
          ...
          checkValidate() {
            if (!this.model) return; // If model is undefined, return directly
            const validated = this.model.validated;
            const {formSubmited, validateOnChange} = this.props;

            if (
              // If minLength or maxLength is configured, switch to timely validation
          ...
          async validate() {
            ...
            (formItemDispatchEvent ?? this.props.dispatchEvent)?.(
              ...
              form?.data ?? this.props.data // The data in the form must be the latest
            );
            return valid;
          }
          ...
          handleChange(
            ...
            if (
              !this.model ||
              // todo Try not to use strongly coupled types in the future.
              ~[
                'service',
                'group',
                'hbox',
          ...
          emitChange(
            ...
            // Controlled because the value of the last props sent is not recorded, so no comparison is made
            ...
            // onFormItemChange may trigger component destruction, and read this.model again to be undefined
            if (!this.model) {
              return;
            }
          ...
          // Compatible with the old version, the new version can directly use onChange.
          ...
          render() {
            ...
            const injectedProps: any = {
              ...
              formItemValue: value, // For compatibility with old versions of Custom Components
              ...
              // render: this.renderChild // If overwritten, the render on the form is used, and the data used in this is relatively old.
              // ! Without this, the rendering of options in the tree will have problems
              // Todo should not be affected in theory, to be confirmed
