...
class Position extends React.Component<any, any> {
  ...
  updatePosition(target: any) {
    ...
    /** Mark the PopOver ancestor of the host element, which is used to determine whether PopOver can be root closed later */
    if (target) {
    ...
    if (!target || !target.offsetWidth) {
      // Re-render to reset the position
  ...
  componentWillUnmount() {
    // When a PopOver is closed, the tag of mounting the parent PopOver needs to be removed
// There may be multiple child PopOvers here, so componentId needs to be added
    if (
      this.parentPopover &&
  ...
  render() {
    ...
    return cloneElement(child, {
      ...props,
      ...arrowPosition,
      // Prevent child offset from being covered by Overlay offset
