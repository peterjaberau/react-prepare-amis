...
export const iRendererStore = StoreNode.named('iRendererStore')
  .props({
    ...
    initedAt: 0, // Initial init time
    updatedAt: 0, // Update time from the server
    pristine: types.optional(types.frozen(), {}), // pristine data may be modified by the default value of the form item, the initApi of the form, etc.
    pristineRaw: types.optional(types.frozen(), {}), // pristine's original value
    upStreamData: types.optional(types.frozen(), {}), // The original data, which is updated only when synchronized from the upstream. Used to determine whether it has changed
