...
export const RootStore = ServiceStore.named('RootStore')
  ...
  .views(self => ({
    get downStream() {
      ...
      if (self.context || self.query || self.globalVarStates.size) {
        ...
        // Add global and globalState to the data chain
        // corresponds to the value and status of the global variable
        if (self.globalVarStates.size) {
          ...
          // Save the value and status of global variables
          ...
          // self.globalData is always that object, so the component always gets the latest one
  ...
  .actions(self => {
    ...
    /**
     * Compare the differences between the new and old variable lists
     * @param vars new variable list
     * @param originVars original variable list
     * @returns Returns a list of variables that were added, updated, and deleted
     */
    ...
    /**
     * Initialize global variables in batches
     */
    ...
    async function bulkGetGlobalVarData(
      ...
      try {
        ...
        for (let key in data) {
          // Return the undefined part of the data without processing
    ...
    // Set global variables and return a Promise
    ...
    // If the corresponding variable is an object, then the value of the variable can be expanded through this
    const modifyGlobalVarValue = (
      ...
      switch (options.op) {
        case 'set':
          value = options.value;
          break;
        // Add the following places for later use
        // case 'merge':
        //   value = {
    ...
    /**
     * Save the value of a single global variable
     */
    ...
    /**
     * Save the value of the global variable
     */
    async function saveGlobalVarValues(key?: string) {
      ...
      for (let varItem of self.globalVars) {
        ...
        if (!state?.touched) {
          ...
          // No key is specified, but autoSave is false and data is not saved
          continue;
        }
    ...
    // Delay saving the value of the global variable
    ...
    return {
      updateContext(context: any) {
        // Because context is not a controlled property, just share the reference directly
        // Otherwise it will trigger re-rendering of child nodes
