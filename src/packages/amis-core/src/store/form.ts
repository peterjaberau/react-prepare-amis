...
export const FormStore = ServiceStore.named('FormStore')
  .props({
    ...
    restError: types.optional(types.array(types.string), []) // errors not mapped to expression items
  })
  .views(self => {
    function getItems() {
      const formItems: Array<IFormItemStore> = [];

      // Find the form item in the child node that is formItem
    ...
    return {
      ...
      /** Get the child elements of InputGroup*/
      get inputGroupItems() {
  ...
  .actions(self => {
    function setValues(
      ...
      // If there is any data change in the data domain, reset it to remove the previous remaining verification message
    ...
    function setValueByName(
      ...
      // If there is no change, it will not run.
    ...
    const saveRemote: (
      ...
      try {
        ...
        // If it fails, modify the data as well, if there is any data.
    ...
    function setFormItemErrors(
      ...
      // Error messages that are not mapped are added after msg and displayed
      !isEmpty(errors) &&
    ...
    // Do not repeat the same error within 10 seconds
    ...
    const validate: (
      ...
      for (let i = 0, len = items.length; i < len; i++) {
        ...
        // Clear the combination check error first
        item.clearError('rules');

        /* Date validation has form item linkage, so you need to reset the validation status before submitting to avoid the linkage validation result not being updated after the variable is updated*/
        if (
          item.validated &&
          isObject(item.rules) &&
          toPairs(item.rules)
        ...
        /**
         * 1. Verified, or unique form item, or mandatory verification, or remote verification API
         * 2. If the default value of the Schema is an expression, you need to recheck it based on the linkage calculation result.
         */
        if (
          !item.validated ||
          item.rules.equals ||
        ...
        if (hooks && hooks.length) {
          for (let i = 0, len = hooks.length; i < len; i++) {
            ...
            if (typeof msg == 'string' && msg) {
              ...
              // Do not prompt and fail the verification
      ...
      if (!self.valid) {
        // If it is not valid and there are form items containing errors that are not remote, it cannot be submitted
        if (
    ...
    function reset(cb?: (data: any) => void, resetData: boolean = true) {
      ...
      // The value may have changed, revalidate.
     ...
     * @param keys specifies that only certain keys are stored
     */
