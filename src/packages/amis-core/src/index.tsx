...
function AMISRenderer({
  ...
  // Compatible with different locale writing methods
  locale =
    locale === 'en'
      ? 'en-US'
      : locale === 'zh' || locale === 'cn'
  ...
  const store = React.useMemo(() => {
    ...
    (window as any).amisStore = store; // For debugging convenience.
    return store;
  ...
  // The mobile native UI will be enabled by default
  if (options.useMobileUI !== false) {
    props.mobileUI = env.isMobile();
  }

  // Overwrite the schema according to the environment. This should be done first, otherwise validations will not be overwritten.
  schema = React.useMemo(() => {
    schema = envOverwrite(schema, locale);
    // todo and envOverwrite are processed together to reduce the number of loops
    schema = replaceText(
      schema,
      options.replaceText,
