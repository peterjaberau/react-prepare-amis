...
export class RootRenderer extends React.Component<RootRendererProps> {
  ...
  constructor(props: RootRendererProps) {
    ...
    //Batch bind the functions in the data to this
    bulkBindFunctions<RootRenderer /*Why can't the type of this be automatically identified? */>(this, [
      'handleAction',
      'handleDialogConfirm',
      'handleDialogClose',
      'handleDrawerConfirm',
  ...
  componentDidMount() {
    ...
    // Compatible with affixOffsetTop and affixOffsetBottom
    if (
  ...
  componentDidUpdate(prevProps: RootRendererProps) {
    ...
    // Must be processed last, otherwise the upper data context in downstream will still be old.
    if (props.data !== prevProps.data || contextChanged) {
      // context depends on data changes to trigger changes, so update it regardless of whether the data has changed
      this.store.initData(props.data);
    }
  }
  ...
  handleAction(
    ...
    if (action.actionType === 'reload') {
      ...
      // Does not support env unless you copy the saveAs code later
  ...
  handleDrawerConfirm(
    ...
    // Wait a moment for the animation to finish.
    setTimeout(() => {
      if (reload) {
        scoped.reload(reload, store.data);
      }
