...
interface ColorPickerProps {
  imageType?: 'image-manager' | 'input-image'; // Image manager, default is input-image
  receiver?: string; // Image upload service
  labelMode?: 'default' | 'input'; // Color block | with input box
  needGradient?: boolean; // gradient
  needImage?: boolean; // Image
  needTheme?: boolean; // Theme color
  needCustom?: boolean; // Custom color
  value?: string;
  options?: any;
  data?: any;
  onChange?: any;
...
function findColor(value: string | undefined, tree: any) {
  ...
  if (typeof value === 'string' && value?.indexOf('var') === 0) {
    ...
    if (r) {
      ...
      res = {label: 'Inherit Conventional', value: value};
    }
  }
  if (value === 'transparent' || value?.includes('none')) {
    res = {label: 'Transparent', value: 'transparent'};
  }
  if (value === 'custom') {
    res = {label: 'Configure separately', value: 'transparent'};
  }
  return res;
}
...
function ThemeColorList(props: ThemeColorProps) {
  ...
  function searchColor(value: string) {
    ...
    let showFlag = false;
    if (value) {
      eachTree(list as any, (item: any, key: any, level: any, paths: any) => {
        if (item.children) {
          item.show =
    ...
    <div className="ThemeColorList">
      <div className="ThemeColorList-content ThemeColorList-content--top">
        ...
        <div className="ThemeColorList-content--search">
          <SearchBox
            placeholder="Enter color value or name to search"
      ...
      {showFlag ? (
        ...
        <span className="ThemeColorList-content-title">No color matched</span>
      )}
    </div>
  );
}
...
function GradientColor(props: ThemeColorProps) {
  ...
  // Press and hold the gradient point
  ...
  function handleMoveSliderItem(e: MouseEvent) {
    ...
    // If the number of gradient points is greater than 2, delete the gradient points when the y-axis movement is greater than 30.
    if (Math.abs(e.y - target.y) > 30 && tempColors.length > 2) {
      tempColors.splice(currentIndex, 1);
      setIndex(0);
      setColors(tempColors);
    }
    // The limit cannot exceed the maximum and minimum values
    else if (itemPosition >= 0 && itemPosition <= 100) {
      tempColors[currentIndex].position = itemPosition;
      // Swap gradient points forward
      if (
      ...
      // Swap gradient points backwards
      else if (
        tempColors[currentIndex + 1] &&
        itemPosition > tempColors[currentIndex + 1].position
      ) {
  ...
  // Calculate the position by angle
  ...
  // Press the gradient angle handle
  ...
  // Move the gradient angle handle
  ...
  // Raise the gradient angle handle
      ...
      >
        <PopOver overlay onHide={close}>
          <div className="Theme-ImageInput-position-container">
            <div className="Theme-ImageInput-position-title">
              Image Position
            </div>
...
function PresetColors(props: {
  ...
  function setPresetColor(value: string) {
    ...
    if (!!~value.indexOf('linear-gradient')) {
      ...
      // The image is not added to the recent color list
      return;
      // if (
  ...
  return (
    <div className="Theme-ColorSelect-PresetColors">
      <div className="Theme-ColorSelect-PresetColors-header">
        <span className="Theme-ColorSelect-PresetColors-title">
          Common colors
        </span>
      ...
      <div>
        {getColorsLen(colors) > 0 ? (
          ...
          <span className="Theme-ColorSelect-PresetColors-content-placeholder">
            Click on the color above to add
          </span>
        )}
      </div>
    </div>
...
function ColorSelect(props: ColorSelectProps) {
  ...
  const tabMap: any = {
    color: {name: 'color', label: 'Theme color'}
  };
  if (needCustom) {
    tabMap['custom'] = {name: 'custom', label: 'Custom color'};
  }
  if (needGradient) {
    tabMap['gradient'] = {name: 'gradient', label: 'gradient'};
  }
  if (needImage) {
    tabMap['img'] = {name: 'img', label: 'Picture'};
  }

  const subTabList: any[] = [
    needTheme &&
      themeList &&
      themeList.length > 0 && {name: 'Theme color', parent: 'color', id: 0},
    needCustom && {name: 'Custom color', parent: 'color', id: 1}
  ].filter(n => n);
