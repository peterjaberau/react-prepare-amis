...
export interface SceneFlexItemStateLike
  extends SceneFlexItemPlacement,
    SceneObjectState {}

export interface SceneFlexItemLike
  extends SceneObject<SceneFlexItemStateLike> {}

interface SceneFlexLayoutState
  extends SceneObjectState,
    SceneFlexItemPlacement {
  children: SceneFlexItemLike[];
}

export class SceneFlexLayout
  extends SceneObjectBase<SceneFlexLayoutState>
  implements SceneLayout
{
...
function SceneFlexLayoutRenderer({
  model,
  parentState,
}: SceneFlexItemRenderProps<SceneFlexLayout>) {
    ...
    <div className={style}>
      {children.map((item) => {
        const Component = item.Component as ComponentType<
          SceneFlexItemRenderProps<SceneObject>
        >;
        return (
          <Component
            key={item.state.key}
...
export interface SceneFlexItemState
  extends SceneFlexItemPlacement,
    SceneObjectState {
...
function SceneFlexItemRenderer({
  model,
  parentState,
}: SceneFlexItemRenderProps<SceneFlexItem>) {
  if (!parentState) {
...
function useLayoutItemStyle(
  state: SceneFlexItemState,
  parentState: SceneFlexItemPlacement,
) {
  return useMemo(() => {
