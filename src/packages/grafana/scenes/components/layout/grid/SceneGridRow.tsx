...
export function SceneGridRowRenderer({
  model,
}: SceneComponentProps<SceneGridRow>) {
  const styles: any = useStyles2(getSceneGridRowStyles as any);
  const { isCollapsible, isCollapsed, title, actions, children } =
    model.useState();
    ...
    <div className={cx(styles.row, isCollapsed && styles.rowCollapsed)}>
      <div className={styles.rowTitleAndActionsGroup}>
        ...
        <span
          className={cx(
            styles.panelCount,
            isCollapsed && styles.panelCountCollapsed,
          )}
        >
          ({count} {panels})
        </span>
