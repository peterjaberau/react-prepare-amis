...
export class EachPlugin extends BasePlugin {
  ...
  description =
    'Functional renderer that can loop over output renderers based on existing variables.';
        ...
        body: getSchemaTpl('collapseGroup', [
          {
            ...
            body: [
              ...
              getSchemaTpl('valueFormula', {
                ...
                name: 'maxLength',
                label: 'Maximum number of displayed items',
                valueType: 'number'
              }),
              getSchemaTpl('valueFormula', {
                rendererSchema: {
  ...
  filterProps(props: any, node: EditorNodeType) {
    // The text element in the list type displays {{formula}} or a custom booth, not the actual value
    props = escapeFormula(props);

    if (!node.state.value) {
      // Display 2 elements in loop editing mode
      // props.value = [{}, {}];
      node.updateState({
        value: [{}, {}]
      });
