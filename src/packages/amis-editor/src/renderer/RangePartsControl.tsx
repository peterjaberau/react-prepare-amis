...
const PartsSourceOptions = [
  {label: 'No block', value: PartsSourceEnum.NO_BLOCK},
  {label: 'Average', value: PartsSourceEnum.AVERAGE},
  {label: 'By step length', value: PartsSourceEnum.STEPS},
  {label: 'Custom', value: PartsSourceEnum.CUSTOM}
];

const MarksSourceOptions = [
  {label: 'Consistent with blocks', value: MarksSourceEnum.PARKS},
  {label: 'Custom', value: MarksSourceEnum.CUSTOM}
];

// Get the block mode according to the slider configuration
...
> {
  ...
  componentDidUpdate(prevProps: MarksControlProps) {
    ...
    if (
      ...
      // Keep consistent with the block, when the block and unit are transformed synchronously, synchronize the subscript
      source === MarksSourceEnum.PARKS && this.onSynchronismParts();
    }
  }

  /**
   * Configure the obtained marks data to be converted into options
   * @param marks
   * @returns
   */
  @autobind
  ...
  /**
   * Different block division methods => different subscript data
   */
  @autobind
  onSynchronismParts() {
    const {
