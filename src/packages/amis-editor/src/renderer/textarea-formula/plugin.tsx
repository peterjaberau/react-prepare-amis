...
import {FormulaEditor} from '@/packages/amis/src';
import type {VariableItem, CodeMirror} from '@/packages/amis/src';
...
interface FormulaPluginConfig {
  ...
  showPopover?: boolean;
  showClearIcon?: boolean; // Expression whether to display the delete icon
}
...
export class FormulaPlugin {
  ...
  autoMark() {
    ...
    for (let line = 0; line < lines; line++) {
      ...
      for (let i = 0; i < braces.length; i++) {
        // Replace the contents of each formula expression
  ...
  // Find the location of the expression
