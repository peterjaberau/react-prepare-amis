...
export class PyroscopeDataSource extends DataSourceWithBackend<Query | any, PyroscopeDataSourceOptions> {
  constructor(
  ...
  query(request: DataQueryRequest<Query | any>): Observable<DataQueryResponse> {
  ...
  async getTagKeys(options: DataSourceGetTagKeysOptions<Query | any>): Promise<MetricFindValue[]> {
  ...
  // By implementing getTagKeys and getTagValues we add ad-hoc filters functionality
  async getTagValues(options: DataSourceGetTagValuesOptions<Query | any>): Promise<MetricFindValue[]> {
    const data = this.adhocFilterData(options);
    const labels = await this.getLabelValues(data.query, options.key, data.from, data.to);
    return labels.map((label) => ({ text: label }));
  }

  private adhocFilterData(options: DataSourceGetTagKeysOptions<Query | any> | DataSourceGetTagValuesOptions<Query | any>) {
  ...
  applyTemplateVariables(query: Query | any, scopedVars: ScopedVars, filters?: AdHocVariableFilter[]): Query {
  ...
  importFromAbstractQuery(labelBasedQuery: AbstractQuery): Query | any {
    return {
  ...
  exportToAbstractQuery(query: Query | any): AbstractQuery {
