...
import { DataSourceApi, getDefaultTimeRange, LoadingState, PanelData, SelectableValue, TimeRange } from '@data/index';
import {
  EditorRow,
  LabelFilters,
...
export const LokiQueryBuilder = memo<Props>(({ datasource, query, onChange, onRunQuery, showExplain, timeRange }) => {
  ...
  useEffect(() => {
    const onGetSampleData = async () => {
      const lokiQuery: any = { expr: lokiQueryModeller.renderQuery(query), refId: 'data-samples' };
  ...
  return (
    <div data-testid={testIds.editor}>
      ...
      <OperationsEditorRow>
        ...
        />
        <QueryBuilderHints<LokiVisualQuery, LokiQuery | any>
