...
} from "./core/components/OptionsUI/registry";
...
} from "./features/plugins/extensions/utils";
import {
  importPanelPlugin,
  syncGetPanelPlugin,
} from "./features/plugins/importPanelPlugin";
...
import {
  getVariableQueryRunner,
  setVariableQueryRunner,
  VariableQueryRunner,
} from "./features/variables/query/VariableQueryRunner";
...
const extensionsIndex = import.meta.glob("./extensions/index.ts", {
  eager: true,
});
  ...
  async init() {
    try {
      ...
      const skipAppPluginsPreload =
        config.featureToggles.rendererDisableAppPluginsPreload &&
      ...
      setPluginExtensionGetter(
        createPluginExtensionsGetter(pluginExtensionRegistries),
      );
      setPluginExtensionsHook(
        createUsePluginExtensions(pluginExtensionRegistries),
      );
      ...
      console.log("Failed to start Grafana: --------: ", error);
...
async function initEchoSrv() {
  ...
  if (contextSrv.user.orgRole !== "") {
    const { PerformanceBackend } = await import(
      "./core/services/echo/backends/PerformanceBackend"
    );
  ...
  if (config.googleAnalyticsId) {
    const { GAEchoBackend } = await import(
      "./core/services/echo/backends/analytics/GABackend"
    );
    registerEchoBackend(
      new GAEchoBackend({
  ...
  if (config.googleAnalytics4Id) {
    const { GA4EchoBackend } = await import(
      "./core/services/echo/backends/analytics/GA4Backend"
    );
    registerEchoBackend(
      new GA4EchoBackend({
        googleAnalyticsId: config.googleAnalytics4Id,
        googleAnalytics4SendManualPageViews:
          config.googleAnalytics4SendManualPageViews,
      }),
    );
  }

  if (config.rudderstackWriteKey && config.rudderstackDataPlaneUrl) {
    const { RudderstackBackend } = await import(
      "./core/services/echo/backends/analytics/RudderstackBackend"
    );
    registerEchoBackend(
  ...
  if (config.analyticsConsoleReporting) {
    const { BrowserConsoleBackend } = await import(
      "./core/services/echo/backends/analytics/BrowseConsoleBackend"
    );
 ...
 */
function reportMetricPerformanceMark(
  metricName: string,
  prefix = "",
  suffix = "",
): void {
  ...
  if (metric) {
    ...
    reportPerformance(
      `${prefix}${metricName}${suffix}`,
      Math.round(metric.startTime) / 1000,
    );
  }
}
...
function handleRedirectTo(): void {
  ...
  if (queryParams.has(redirectToParamKey) && window.location.pathname !== "/") {
    ...
    window.history.replaceState(
      {},
      "",
      `${window.location.pathname}${queryParams.size > 0 ? `?${queryParams}` : ""}`,
    );
    return;
  }
