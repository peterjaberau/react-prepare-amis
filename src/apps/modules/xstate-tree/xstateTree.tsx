 ...
 * @internal
 */
export function recursivelySend(service: AnyActorRef | any, event: GlobalEvents | any) {
  ...
  const RootComponent = function XstateTreeRootComponent() {
    ...
    const [_, __, interpreter] = useActor(machine, {
      inspect(event) {
        switch (event.type) {
          case "@xstate.actor":
            console.log(`[xstate-tree] actor spawned: ${(event.actorRef as any).id}`);
            break;
          case "@xstate.event":
            console.log(
              `[xstate-tree] event: ${
                event.sourceRef ? (event.sourceRef as any).id : "UNKNOWN"
              } -> ${event.event.type} -> ${(event.actorRef as any).id}`,
              event.event
            );
            break;
          case "@xstate.snapshot":
            ...
            if (!lastSnapshot) {
              console.log(
                `[xstate-tree] initial snapshot: ${(event.actorRef as any).id}`,
                toJSON(event.snapshot)
              );
            } else {
              console.log(
                `[xstate-tree] snapshot: ${(event.actorRef as any).id} transitioning to`,
                toJSON(event.snapshot),
                "from",
                toJSON(lastSnapshot)
              );
    ...
    useEffect(() => {
      if (routing) {
        const unsub = routing.history.listen((location: any) => {
          const result = handleLocationChange(
            routing.routes,
            routing.basePath,
            location.pathname,
