...
export function useCreateOrUpdateRepository(name?: string) {
  ...
  const updateOrCreate = useCallback(
    (data: RepositorySpec) => {
      ...
      return create({
        repository: { metadata: generateRepositoryMetadata(data), spec: data },
      });
    },
    [create, name, update],
  );
...
const generateRepositoryMetadata = (data: RepositorySpec) => {
  ...
  const normalisedName = data.title
    .toLowerCase()
  ...
  if (
    ...
    const shortenedName = normalisedName.substring(
      0,
      63 - 1 - randomBit.length,
    );
