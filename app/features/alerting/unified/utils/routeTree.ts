...
import {
  insertAfterImmutably,
  insertBeforeImmutably,
} from "@data/utils/arrayUtils";
import {
  ROUTES_META_SYMBOL,
  Route,
  RouteWithID,
...
export const mergePartialAmRouteWithRouteTree = (
  ...
  function findAndReplace(currentRoute: RouteWithID): Route {
    ...
    if (currentRoute.id === partialFormRoute.id) {
      const newRoute = formAmRouteToAmRoute(
...
export const addRouteToReferenceRoute = (
  ...
  return produce(routeTree, (draftTree) => {
    const [routeInTree, parentRoute, positionInParent] = findRouteInTree(
      draftTree,
      referenceRouteIdentifier,
    );

    if (
      routeInTree === undefined ||
      parentRoute === undefined ||
      positionInParent === undefined
    ) {
      throw new Error(
    ...
    if (position === "above") {
      parentRoute.routes = insertBeforeImmutably(
        parentRoute.routes ?? [],
        newRoute,
        positionInParent,
      );
    ...
    if (position === "below") {
      parentRoute.routes = insertAfterImmutably(
        parentRoute.routes ?? [],
        newRoute,
        positionInParent,
      );
...
export function findRouteInTree(
  ...
  parentRoute: RouteMatch,
  positionInParent: number | undefined,
] {
  ...
  function findRouteInTree(
    ...
    if (currentRoute.routes) {
      currentRoute.routes.forEach((route, index) =>
        findRouteInTree(route, index, currentRoute),
      );
    }
  }
...
export function findExistingRoute(
  ...
  return routeTree.id === id
    ? routeTree
    : routeTree.routes?.find((route) => findExistingRoute(id, route));
}
...
export function stabilizeRoute(route: Route): Required<Route> {
  const result: Required<Route> = {
    ...
    mute_time_intervals: route.mute_time_intervals
      ? [...route.mute_time_intervals].sort()
      : [],
    active_time_intervals: route.active_time_intervals
      ? [...route.active_time_intervals].sort()
      : [],
